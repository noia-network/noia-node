...
import { Statistics, StatisticsOptions } from "./lib/statistics"
...
class Node extends EventEmitter {
  ...
  start (opts?: any) {
    ...
    if (skipBlockain) {
      ...
      this.wallet.lazyNodeRegistration(this.settings.get(Options.client))
        .then((isRegistered) => {
          if (isRegistered) {
            this.wallet.findFirstJob()
              .then((jobData: any) => {
                const address = `ws://${jobData.info.host}:${jobData.info.port}`
                this.master.connect(address, jobData.employerAddress)
              })
              .catch((err: Error) => {
                logger.error("Could not find job and connect to master", err)
                throw new Error(err.message)
              })
          } else {
            logger.info("Node failed to register. Will try again.")
            setTimeout(() => {
              this.restart()
            }, 15000)
          }
